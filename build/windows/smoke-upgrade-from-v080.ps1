...
try {
  ...
  Health '1.0.0-rc.3'
  if((Get-FileHash $sentinel -Algorithm SHA256).Hash-ne$hash){throw 'Sentinel changed during v0.8-to-v1.0-rc.3 upgrade.'}
  $again=Api POST '/api/login' @{username=$User;password=$Changed}; if(-not $again.token){throw 'Changed credential not preserved.'}
  Write-Host 'PASS: real v0.8.0-rc.1 -> v1.0.0-rc.3 installer upgrade preserved service, TLS, database and credentials.'
