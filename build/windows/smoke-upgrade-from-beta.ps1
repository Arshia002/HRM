...
$Data=Join-Path $env:ProgramData 'HRM-Kermanshah'; $Api='https://127.0.0.1:8765'; $User='arshia.shahbazi'; $Changed='RC-Upgrade!Password1401'
...
try {
  ...
  Install (Resolve-Path $BaselineInstaller).Path (Join-Path (Split-Path $LogPath) 'beta-install.log')
  Health '0.6.0-beta.1'
  ...
  $sentinel=Join-Path $Data 'ci-beta-to-rc-sentinel.txt'; Set-Content $sentinel 'preserve-beta-to-rc' -Encoding ASCII; $hash=(Get-FileHash $sentinel -Algorithm SHA256).Hash
  Install (Resolve-Path $CandidateInstaller).Path (Join-Path (Split-Path $LogPath) 'rc-upgrade.log')
  Health '0.7.0-rc.1'
  if((Get-FileHash $sentinel -Algorithm SHA256).Hash-ne$hash){throw 'Sentinel changed during beta-to-RC upgrade.'}
  $again=Api POST '/api/login' @{username=$User;password=$Changed}; if(-not $again.token){throw 'Changed credential not preserved.'}
  Write-Host 'PASS: real v0.6.0-beta.1 -> v0.7.0-rc.1 installer upgrade preserved service, TLS, database and credentials.'
