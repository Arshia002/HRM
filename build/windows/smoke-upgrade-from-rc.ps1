...
$Data=Join-Path $env:ProgramData 'HRM-Kermanshah'; $Api='https://127.0.0.1:8765'; $User='arshia.shahbazi'; $Changed='V080-Upgrade!Password1401'
...
try {
  ...
  Install (Resolve-Path $BaselineInstaller).Path (Join-Path (Split-Path $LogPath) 'v070-install.log')
  Health '0.7.0-rc.1'
  ...
  $sentinel=Join-Path $Data 'ci-v070-to-v080-sentinel.txt'; Set-Content $sentinel 'preserve-v070-to-v080' -Encoding ASCII; $hash=(Get-FileHash $sentinel -Algorithm SHA256).Hash
  Install (Resolve-Path $CandidateInstaller).Path (Join-Path (Split-Path $LogPath) 'v080-upgrade.log')
  Health '0.8.0-rc.1'
  if((Get-FileHash $sentinel -Algorithm SHA256).Hash-ne$hash){throw 'Sentinel changed during v0.7-to-v0.8 upgrade.'}
  $again=Api POST '/api/login' @{username=$User;password=$Changed}; if(-not $again.token){throw 'Changed credential not preserved.'}
  Write-Host 'PASS: real v0.7.0-rc.1 -> v0.8.0-rc.1 installer upgrade preserved service, TLS, database and credentials.'
