...
if "%~1"=="" (
  ...
  echo Usage: INSTALL-OVERLAY-V100RC1.cmd "C:\path\to\HRM-repository"
  exit /b 1
)
...
if errorlevel 1 (
  echo ERROR: RC branch is not based on the tested v0.8.0-rc.1 source baseline.
  exit /b 1
)
...
echo Removing explicitly obsolete tracked test artifacts from the final-candidate worktree...
python "%HRM_REPOSITORY%\ci\cleanup_v100rc1_repository.py" --root "%HRM_REPOSITORY%"
if errorlevel 1 (
  echo ERROR: final-candidate repository hygiene cleanup failed.
  exit /b 1
)
