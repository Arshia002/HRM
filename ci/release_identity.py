#!/usr/bin/env python3
"""Single release-identity contract for the current HRM final production release-candidate overlay."""
...
METADATA_FILE = ROOT / "VERSION-V100RC1.json"
...
def load_identity() -> ReleaseIdentity:
    ...
    if (ROOT / "VERSION").read_text(encoding="utf-8").strip() != version:
        raise RuntimeError("VERSION does not match VERSION-V100RC1.json")
    if (ROOT / "CI-PACKAGE-VERSION").read_text(encoding="utf-8").strip() != package_revision:
        raise RuntimeError("CI-PACKAGE-VERSION does not match VERSION-V100RC1.json")
