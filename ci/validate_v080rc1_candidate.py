...
def main() -> int:
    ...
    if identity.version != "0.8.0-rc.1" or identity.package_revision != "0.8.0-rc.1-ci.1":
        fail("RC identity is not the approved v0.8 ci.1 release identity")
    if identity.baseline_commit != "2a1a7f7b71b3a01c9f459f17d13d9dc348f45fb2":
        fail("v0.8 RC must remain anchored to the tested v0.7 source revision")
    ...
    required = (
        ...
        "INSTALL-OVERLAY-V080RC1.cmd",
        "build/windows/smoke-upgrade-from-rc.ps1",
    ...
    print("PASS: HRM v0.8.0-rc.1 production-history and Linux-web candidate gates passed.")
    return 0


if __name__ == "__main__":
    if sys.argv[1:] == ["--dependency-self-check"]:
        print("PASS: v0.8 RC validator direct-import contract")
        raise SystemExit(0)
    if sys.argv[1:] == ["--source-path-self-check"]:
        ...
        print("PASS: v0.8 RC validator clean source-path contract")
        raise SystemExit(0)
