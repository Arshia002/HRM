# HRM v0.6.0-beta.1 ci.5 — Protected Organizational Pilot Candidate
...
بازانتشار `ci.3` تست حریم خصوصی را قطعی می‌کند: مقادیر قابل‌نمایش از
فیلدهای SHA-256 جدا بررسی می‌شوند تا تطابق تصادفی یک شماره کوتاه داخل Hash
باعث شکست احتمالاتی Runner نشود.

بازانتشار `ci.5` قبل از بازسازی Manifest، تمام فایل‌های Extract‌شده را با
هش اصلی ZIP تطبیق می‌دهد تا Overlay ترکیبی از دو نسخه فوراً و با نام فایل
ناسازگار متوقف شود.
